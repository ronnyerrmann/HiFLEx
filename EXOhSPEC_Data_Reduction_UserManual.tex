...
\begin{itemize}\setlength\itemsep{0em}
  ...
  \item Improvment of the way how the new header values are stored.
\end{itemize}
% ------------------------------------------------------------------------------
\newpage
...
To install Serval please follow the instructions as given on\\ \url{https://github.com/mzechmeister/serval}~. It is worth to check that the software is working by running the package on the provided test data.
...
\end{lstlisting}
The order and pixel position of all identified lines are stored in the file given in parameter \verb|logging_found_arc_lines| (see Table~\ref{Tab:create_wavelength_solution}. This file can be opened (easiest in a spreadsheet application) and the corresponding wavelengths can be added to some of the lines. The lines for each order are sorted by their absolute height, the saturated lines are excluded. The plots of the extracted spectra, which are stored in the file given in \verb|logging_arc_line_identification_spectrum| (extended by the pipeline with \textit{\_manual.pdf}) can be used for visual guiding (for an example see Figure~\ref{Figure:create_wavelength_solution}). 
...
\begin{figure} 
  ...
  {\includegraphics[height=4cm]{./figures/create_wavesol_unknow_spec_MRES_o22}
  }
  \qquad
  ...
  {\includegraphics[height=4cm]{./figures/create_wavesol_know_spec_UH_o26.png}
  }
  \caption{The emission lines for a short (orange) and a long (blue) exposure time. The left figure covers about the same wavelength range as the Th-Ar spectrum with wavelength solution in the right figure. The right image can be used to correlate pixel-positions and wavelengths in the left spectrum.
    \label{Figure:create_wavelength_solution}}
\end{figure}

\begin{table}
  \caption{Transforming the emission lines found by the pipeline into the file from which the pipeline will create the wavelength solution.}
...
22 & 73.44 & 0.99 & 50.2 \\
    \end{tabular}}
  \qquad
  \subfloat[Example of \textit{arc\_lines\_wavelength.txt} for one order. Lines without a wavelength can, but don't have to be deleted. The text in the columns with line type and afterwards are optional. Columns have to be spaced by tabulator.]{
...
The correlated data needs to be saved into file \textit{arc\_lines\_wavelength.txt} (in the working directory) with the following tab-separated entries:
\begin{itemize}
  \item Aperture (starting at 0 for the reddest order)
  \item Real/physical order (bigger than 0, usually between 40 and 120, doesn't have to be exactly the right one)
  \item Pixel
