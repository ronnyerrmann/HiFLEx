...
\begin{enumerate}
  ...
  \begin{itemize}[leftmargin=*]\setlength\itemsep{0em}
    ...
    \item Adjust \verb|rotate_frame| and \verb|flip_frame| so that the orders are up to down (blue wavelength on top) and red orders are on the right. The pipeline will first rotate and then flip, the flip is swapping left and right.
  ...
  \item Run the scrip \verb|prepare_file_list.py| (see Chapter~\ref{Section:prepare_files} below for more information): \vspace*{-\itemsep}
  \begin{itemize}[leftmargin=*]\setlength\itemsep{0em}
...
\subsection{Steps to run the code on a new set of data - to be moved to the front}
