...
Please found the changes of older versions in Chapter~\ref{Section:Changelog_old}.
...
\begin{enumerate}
  ...
  \begin{itemize}[leftmargin=*]\setlength\itemsep{0em}
    ...
    \item Modify the header-parameters, if necessary (see Cahpter~\ref{Section:prepare_files}).
  ...
  \item Create a new wavelength solution (in case no wavelength solution is required, set \verb|master_wavelensolution_filename| to \verb|pseudo| and skip the other steps under this bullet point): \vspace*{-\itemsep}
  \begin{itemize}[leftmargin=*]\setlength\itemsep{0em}
    ...
    \item To find a wavelength solution a file which provides the wavelength for some (extracted) pixel and wavelengths needs to be given (called \textit{pixel\_to\_wavelength.txt}. Until a GUI is available within the pipeline, a way to create the list is described in Chapter~\ref{section:create_new_wave_solution}.
...
\begin{itemize}
  ...
  \item Run the scrip \verb|prepare_file_list.py| to define what files should be used for what calibration.
    
  \item Run the scrip \verb|reduction_day.py|.
...
\noindent An example for the parameters with additional explanation is shown in Figure~\ref{Fig:example_calibs_create_g}.
...
After checking the results (see Chapter~\ref{section:results_pipeline}), one might want to remove wrongly identified apertures. If a trace is located close to the borders this can lead to wrongly traced orders, which can't be handled by the code automatically due to it's big flexibility. The orders can be removed in a graphical user interface by running \verb|remove_orders.py|. The old files will be moved into a sub-folder and the depending steps of \verb|reduction_day.py| will run again.
