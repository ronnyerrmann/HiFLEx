...
\title{User Manual for Data Reduction using the HiFLEx package\\ version: v1.5.1}
\date{July, 2021}
...
\subsection{Processing doesn't continue (without crashing)}
If the total memory is used during processing while multiprocessing is used, some of the processes can get stuck and the pipeline halts there without doing anything. The solution is to kill the current instance of HiFLEx and restart it. Maybe the parameter \verb|max_cores_used_pct| needs to be set to a lower value.

\subsection{Running out of storage}
The following commands can be used to clear up files, that the user might not need

\begin{lstlisting}[style=base]
rm */extracted/single/*_lin_cont.csv
\end{lstlisting}
...
Some external packages (CERES) only work under Python~2.7. Therefore, if you want to use these packages, it is necessary to create a python~2 environment, called (hiflex\_p2):
\begin{lstlisting}[style=base]
conda deactivate
...
\paragraph{Manual installation of barycorrpy }
...
\begin{lstlisting}[style=base]
wget https://github.com/ronnyerrmann/HiFLEx/archive/v1.5.0.tar.gz
tar -xzvf v*.tar.gz
\end{lstlisting}
...
\begin{description}
 \item[conf.txt] Example configuration file with explanations.
 ...
 \item[remove\_add\_modify\_orders.py.py] If the automatic process identified wrong orders, then this script allows the user to remove or add or modify them using a GUI.
 \item[create\_badpx\_mask.py] Script to create a bad pixel mask (in testing).
 \item[measurements\_to\_output.py] Reads the header information of the extracted files and stores the results in a \verb|csv|-file
 \item[plot\_measurement\_table.py] Reads the \verb|csv|-file and plots the results.
\end{description}
...
Set \verb|terra_jar_file| in \textit{conf.txt} to the correct path.
...
Set \verb|path_serval| in \textit{conf.txt} to the correct path.

If the SERVAL RV analysis fails, try to run it with a subset of orders for the \verb|oset| parameter. The logfile and cmdhistory.txt in the folder given in \verb|path_rv_serval| give the commands that were run automatically by the pipeline. Before running SERVAL manually, it might be necessary to the path to PYTHONPATH. The example below shows the commands for \verb|bash| and \verb|csh| to do this:
...
Set \verb|path_ceres| in \textit{conf.txt} to the correct path.
...
\subsection*{v1.4.2}
\begin{itemize}\setlength\itemsep{0em}
  \item Removed the manual IERS addresses which were introduced in May 2020, disabled ssl verification if download of IERS fails.
  \item Fixed crash when using old matplotlib.
\end{itemize}
