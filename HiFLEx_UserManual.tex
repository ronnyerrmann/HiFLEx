...
\subsubsection{TERRA}
\paragraph{Files and folders in folder given in parameter path\_rv\_terra (standard: terra\_rv}
\label{Section:files_folders_rv_for_terra}
The data of the individual objects are stored in this folder in the subfolder \textit{object name + /data/}. If TERRA is installed and \verb|terra_jar_file| is set correctly to the TERRA \textit{PRV.jar}-file, then TERRA will run on each \textit{object name} within the folder. The results of the radial velocity analysis will be stored in \textit{object name + /results/synthetic.rv}.
