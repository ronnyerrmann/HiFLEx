...
\title{User Manual for Data Reduction using the HiFLEx package\\ version: v1.3.3}
\date{February, 2021}
...
The automatic process to trace orders can identy too many orders. However, in low light areas it can also be that the traces stop prematurely, while strong emission features are still detectedable in the science images, hence the user might want to extend the orders. In case of imaging the spectra on multiple detectors, orders might fall between the detectors, hence a dummy order needs to be added later, as the wavelength solution requires consecutive orders. All this is possible in a GUI, as seen in Figure~\ref{Fig:GUI_remove_add_modify_orders}. If \verb|GUI=True|, then the GUI will be opened automatically, otherwise it can be openend by running \verb|python <path_hiflex>/remove_add_modify_orders.py|.
...
\begin{lstlisting}[style=base]
wget https://github.com/ronnyerrmann/HiFLEx/archive/v1.3.3.tar.gz
tar -xzvf v*.tar.gz
\end{lstlisting}
