...
\begin{enumerate}\setlength\itemsep{0em}
  \item Find an order close to the cental order with at least 2 prominent lines for which the wavelength is know, best several 100 or 1000 pixel apart of each other by changing the order number and \textit{Update}.
...
% not useful  \item[3.] Creating a map of the leaking light between the science orders (background map).
  ...
  \begin{itemize}
    ...
    \item[e)] Finding the offset between the emission lines in the calibration spectra and the lines used for the wavelength solution (for bifurcated fiber spectrographs). Additionally, interpolating the measured drift from step 6 to the mid exposure time. Calculating the wavelength for each pixel in the extracted spectrum.
