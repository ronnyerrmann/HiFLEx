...
def textfileargs(params, textfile=None):
    ...
    # deal with lists
    lists = ['subframe', 'arcshift_range', 'order_offset', 'px_offset', 'px_offset_order', 'use_catalog_lines', 'polynom_order_traces', 'polynom_order_intertraces', 'opt_px_range', 'bin_search_orders', 'bin_adjust_orders', 'raw_data_file_endings']
    for entry in params.keys():
    ...
    lists = ['subframe', 'arcshift_range', 'order_offset', 'px_offset', 'px_offset_order', 'polynom_order_traces', 'polynom_order_intertraces', 'bin_search_orders', 'bin_adjust_orders']
    for entry in lists:
    ...
    # deal with ints
    floats = ['polynom_order_orders', 'rotate_frame', 'polynom_bck']
    for entry in floats:
        try:
...
def read_background(params, filename): 
    ...
    if os.path.isfile(filename) == False:
        logger('Error: File {0} is missing -> exit'.format(filename))
    im  = np.array(fits.getdata(filename))
    im_head = fits.getheader(filename)
    exptime = im_head['EXPTIME']
    im = rotate_flip_frame(im, params )
    if 'subframe' in params['calibs_read']:
        subframe = params['subframe']
        im = im[subframe[2]: subframe[0]+subframe[2], subframe[3]: subframe[1]+subframe[3]]
        logger('Info: subframe applied: {0}'.format(subframe))
    background = im/exptime"""
...
def read_file_calibration(params, filename, level=0):
    ...
    im  = np.array(fits.getdata(filename), dtype=np.float64)
    im_head = fits.getheader(filename)
    ...
    for entry in params['calibs']:
        ...
        if entry.lower().find('subframe') > -1:
            ...
            if entry not in calimages:
                 calimages[entry] = read_background(params, params[entry])
            warn_images_not_same([im, calimages[entry]], [filename,entry])
        ...
        if logtxt <> [] and headtxt <> []:
            im_median, im_std = int(round(np.median(calimages[entry]))), int(round(np.std(calimages[entry])))
...
def save_multispec(data, fname, head):
    """
            ...
            for i in range(xord+1):
                if i+j == k:
                    ...
                    if len(ims) > 1:                # If the input is an array of values
                        ...
                        a = np.append(a, data)      # If the input is a single values
...
def polynomial_fit_2d_norm(xx, yy, zz, xord, yord, w=[]):
    ...
    if len(w) > 0:
        ...
        #w = ((w/min(w))**2 * 10).astype(int)       # Normalise so that the smallest weight has a value of 10, but this increaese the array so massively
        w = ((w/min(w))).astype(int)
        #print min(w),max(w), sum(w)
        for i in range(w.shape[0])[::-1]:
            A = np.concatenate( (A, A[[i],:].repeat(w[i]-1,axis=0)) , axis=0)
            zz = np.concatenate( (zz, zz[[i]].repeat(w[i]-1,axis=0)) , axis=0)
...
def sigma_clip(xarr, yarr, p_orders, sigma_l, sigma_h, repeats = 1):
    ...
    for i in range(repeats):
        ...
        stddiff = np.std(yarr[goodvalues] - np.polyval(poly, xarr[goodvalues]), ddof=1)
...
def centroid_order(x,y,center,width, significance=3, bugfix=False):
    ...
    try:
        ...
        return [0,0,0,0]
    stdfit = np.std(oneD_gauss(x,popt)-y, ddof=1)
    stdlin = np.std(y, ddof=1)
    #print 'stdfit,stdlin', stdfit,stdlin,stdlin/stdfit < significance, popt[0]/stdfit < significance, popt
    if stdlin/stdfit < significance or popt[0]/stdfit < significance:   # fit is not significant
        if bugfix:
            print 'Gauss not significant', p0,bounds
...
def find_center(imslice, oldcenter, x, maxFWHM, significance=3.0, bugfix=False):
    ...
    if width > maxFWHM or width == 0:                                        
        ...
        popt = centroid_order(range(leftmin,rightmin+1), imslice[leftmin:rightmin+1], center, width, significance=significance, bugfix=bugfix)      # change to 3.5 for the new CCD, less doesn't make sense
...
def measure_noise(spectra, p_order=12, semi_range=10):
    ...
    for order in range(specs[0]):
        ...
        for index in xarr:
            ...
            if len(res_sub) >= 3:
                noise_order[index] = np.std(res_sub, ddof=1)
...
def find_trace_orders(params, im):
    """
    :param params: Dictionary with all the parameters. 'binx', 'biny', 'subframe', and 'polynom_order_orders' are required
    :param im: 2d numpy array
    ...
    ims = im.shape
    cen_px = ims[0]*binx/2
    breakvalue = np.percentile(im,20)       # The brightes pixel of one order needs to be brighter than this value
    ...
    for dummy in tqdm(range(ims[0]*ims[1]/(100/binx*2*maxFWHM*2))):
        ...
        if pos_max[0] < 100/binx or pos_max[0] > ims[0]-100/binx or pos_max[1] < 50/biny or pos_max[1] > ims[1]-50/biny:
        ...
        for i in range(pos_max[0],-1,-1):               # check positions upwards
            center, width, leftmin,rightmin = find_center(im_orig[i,:], int(round(oldcenter)), i, maxFWHM, significance=4.0)       # significance=4.0 tested as useful
            #if pos_max[1] > 477 and pos_max[1] < 500:
            #    print pos_max, i, center, width, leftmin,rightmin, oldcenter, abs(center-oldcenter), maxshift, len(positions)
            if width <> 0 and ( abs(center-oldcenter)<maxshift or (positions==[] and abs(center-oldcenter)<maxshift*2) ):        #first entry can be further off
                ...
                no_center += 1
                if no_center >= max(3,150/binx):         # stop searching if too many fits are unseccessful, as otherwise the fit might drift off
                    break
                if lastadd-i == 10 and expected_positions <> []:        #add entries every 10 empty line in order to avoid the fit of the trace going off
                    positions.append([i, oldcenter, 1])
                    lastadd = i
                if positions <> []:
                    oldcenter = positions[last_trustworth_position][1] - expected_positions[positions[last_trustworth_position][0]] + expected_positions[max(0,i-1)]
        if positions <> []:
        ...
        for i in range(pos_max[0]+1,ims[0]):               # check positions downwards
            #if pos_max[1] > 477 and pos_max[1] < 500:
            #    center, width, leftmin,rightmin = find_center(im_orig[i,:], int(round(oldcenter)), i, maxFWHM, significance=4.0, bugfix=True)       # significance=4.0 tested as useful
            ...
            center, width, leftmin,rightmin = find_center(im_orig[i,:], int(round(oldcenter)), i, maxFWHM, significance=4.0)       # significance=4.0 tested as useful  
            if width <> 0 and ( abs(center-oldcenter)<maxshift or (positions==[]and abs(center-oldcenter)<maxshift*2) ):
                ...
                if no_center >= max(3,150/binx):         # stop searching if too many fits are unseccessful, as otherwise the fit might drift off
                    break
                if i-lastadd == 10 and expected_positions <> []:
                    positions.append([i, oldcenter, 1])
                    lastadd = i
                if positions <> []:
                    oldcenter = positions[last_trustworth_position][1] - expected_positions[positions[last_trustworth_position][0]] + expected_positions[min(ims[0]-1,i+1)]
        ...
        pfs, ns = polynomialfit(positions[:,0]*binx, positions[:,1]*biny, 1, params['polynom_order_orders'])
    ...
    if traces.shape[0] < 1:
        logger('Error: Not enough traces found, only found {0} traces.'.format(traces.shape[0]))
...
def adjust_trace_orders(params, im, pfits, xlows, xhighs):
    ...
    for order in tqdm(range(trace_pos.shape[0]-1)):
        ...
        pfs = np.polyfit(positions[:,0]*binx, positions[:,1]*biny, params['polynom_order_orders'])
        polyfits.append(pfs)
...
def extract_orders(params, image, pfits, xlows, xhighs, widths, w_mult, offset = 0, var='fast'):
    ...
    for pp, pf in tqdm(enumerate(pfits)):
        ...
        for xrow in xarr1[order_in_frame]:
            ...
            #    print y
            #    print diff/y
            #    print np.mean(diff), np.std(diff, ddof=1), np.sum(diff),
            #    diff = np.abs(diff)
            #    print np.mean(diff), np.std(diff, ddof=1), np.sum(diff)
            #poly = np.poly1d(p)
            #polyint = poly.integ()
...
def bck_px(im, pfits, xlows, xhighs, widths, w_mult):
    """
    ...
    for pp,pfit in enumerate(pfits):
        xarr = range(max(0,xlows[pp]-50),min(ims[0],xhighs[pp]+50))
...
def arc_shift(params, im_arc, pfits, xlows, xhighs, widths):
    ...
    :param params: Dictionary with all the paramerters. arcshift_range, arcshift_side, and arcextraction_width_multiplier will be used
    ...
    if len(pfits) >= 10:
        ...
        params['arcshift_range'] = [int(np.median(widths[:,2])*2), int(np.max([np.median(cen_pos_diff[:5]), np.median(cen_pos_diff[-5:])]) - np.median(widths[:,2])*2 + 1) ]  # Test everything between the 3 times Gauss width and next Order-3*width
        cen_pos_diff = np.insert(cen_pos_diff, 0, cen_pos_diff[0])                      # create the same length array
    arcshifts = np.arange(min(params['arcshift_side']*np.array(params['arcshift_range'])),max(params['arcshift_side']*np.array(params['arcshift_range']))+1)
    fluxes = []
    ...
    for order in orders:
        if cen_pos_diff <> []:
            ...
            if len(x) < 5:                      # not enough values, use standard settings
                goodpos = ( (np.abs(arcshifts) > min(params['arcshift_range'])) & (np.abs(arcshifts) < max(params['arcshift_range'])) )
                x, y = arcshifts[goodpos], fluxes[goodpos,order]
        else:
            x, y = arcshifts, fluxes[:,order]
        popt = centroid_order(x,y,np.mean(x),max(x)-min(x))    #a,x0,sigma,b in a*np.exp(-(x-x0)**2/(2*sigma**2))+b
        gauss.append(popt)      #shift=popt[1]
...
def read_reference_catalog(filename, wavelength_muliplier, arc_lines):
    ...
    :return reference_catalog: 2d array with one entry for each line. Each entry contains the wavelength and the intensity of the line
    ...
    for line in file:
        ...
        if line[2].replace(' ','') not in arc_lines:
            continue
        reference_catalog.append(line[0:2])   # wavelength in Angstrom, relative intensity of the line
...
def shift_wavelength_solution(aspectra, wavelength_solution, wavelength_solution_arclines, reference_catalog, reference_names, xlows, xhighs):
    ...
    FWHM = 6
    in_shift = 0       #0 
    ratio_lines_identified = 0.02       # if more than ratio_lines_identified of the checked_arc_lines has been identified, then a sigma_clipping will be applied. If less than this number of lines remain after sigma clipping, then the assumption is, that the calibration fiber wasn't used and therefore no wavelength shift is applied
...
def trace_orders(params, im_sflat, im_sflat_head):
    ...
    :return widths: 2d array of floats, length same as number of orders, width of the trace in cross-dispersion direction. 
                    Each line contains the giving the width to the left, to the right, and the Gaussian width
    """
    logger('Setp: Tracing the orders')
    if os.path.isfile(params['master_order_filename']) == True:
        ...
        if os.path.isfile(params['logging_traces_binned']) == True:
            ...
            params['binx'], params['biny'] = params['bin_search_orders']
            sim_sflat = bin_im(params, im_sflat)
            save_im_fits(params, sim_sflat, im_sflat_head, params['result_path']+'flat_binned.fits')
        ...
        params['binx'], params['biny'] = params['bin_adjust_orders']
        sim_sflat = bin_im(params, im_sflat)
...
def extraction_steps(params, im, im_name, im_head, polyfits, xlows, xhighs, widths, apolyfits, axlows, axhighs, awidths, wavelength_solution, wavelength_solution_arclines, reference_catalog, reference_names, flat_spec_norm, im_flat):
    ...
    save_multispec([wavelengths, spectra, espectra, fspectra, efspectra, cspectra, sn_cont, good_px_mask, aspectra], params['path_extraction']+im_name, im_head)
    """
...
def bck_fit(im, im_bck_px, p_orders, GUI=True):         # Needs to be rewritten using 2d image fit.
    ims = im.shape
    im_bck = []
    ...
    for i in range(ims[0]):           #crossing the orders
        ...
        sigmaclip = (yarr*0 == 0)
        for j in range(3):
            pbck = polyfit_adjust_order(xarr1[sigmaclip], yarr[sigmaclip], p_orders)
            difffit = yarr - np.polyval(pbck, xarr1)
            stddiff = np.std(difffit, ddof=1)
    ...
    for i in range(ims[1]):         #crossing the fits
        pbck = polyfit_adjust_order(yarr, im_bck[:,i], p_orders)
...
def fit_poly_between_orders(polyfit_pl, polyfit_o, polynom_order_intertrace, sigma):       # only used by correlate_px_wavelength, which is only used by ident_arc.py
    ...
    for i in range(available_orders):
        ...
        #print 'i,p, [len(temp), np.std(temp, ddof=1),min(temp),max(temp)]',i,p, [len(temp), np.std(temp, ddof=1),min(temp),max(temp)]
...
def remove_bad_identified_lines(arc_lines_wavelength, polyfit_o, polyfit_p, cen_px, sigma_diff_fit):       # only used by correlate_px_wavelength, which is only used by ident_arc.py
    ...
    std_diff_fit = np.std(arc_lines_wavelength[(arc_lines_wavelength[:, 3]<>0), 3], ddof=1) * sigma_diff_fit
...
def correlate_px_wavelength(im, orig_arc_lines_wavelength, reference_catalog, reference_names, arc_lines_px, polynom_order_trace, polynom_order_intertrace, max_diff_px, sigma_diff_fit):       # only used by ident_arc.py
    ...
    for cen_px_curv in cen_px_curv_range:
        for cen_px_offset in cen_px_offset_range:
            ...
            while True and iteration < iter_break:
                ...
                for order in range(int(np.max(arc_lines_wavelength[:,0]))+1):    #only until the highest order with data
                    ...
                    if xarr.shape[0] >= polynom_order_trace+2:
                        ...
                        #print 'order,p, [len(diff_fit), np.std(diff_fit, ddof=1),min(diff_fit),max(diff_fit)]', order,p, [len(diff_fit), np.std(diff_fit, ddof=1),min(diff_fit),max(diff_fit)]
            ...
            #print 'p_real_cent, [len(diff_real_cent), np.std(diff_real_cent, ddof=1),min(diff_real_cent),max(diff_real_cent)]',p_real_cent, [len(diff_real_cent), np.std(diff_real_cent, ddof=1),min(diff_real_cent),max(diff_real_cent)]
            text = ''
            if order_grating_equation > 1:
                text = '(including distortion, order = {0}) '.format(order_grating_equation)
            logger('Info: the offset to real orders is {0}. With this offset, the standard deviation between the central wavelengths and the grating equation {2}is {1} Angstrom'.format(order_offset, round(np.std(diff_real_cent, ddof=1),4),text) )
            ...
            for order in orders:    #only until the highest order with data
                ...
                if xarr.shape[0] >= polynom_order_trace+2:
                    ...
                    p = np.polyfit(xarr, yarr, polynom_order_trace)      #lambda from px
                    diff_fit = yarr-np.polyval(p, xarr)
                    #print order,p, [len(diff_fit), np.std(diff_fit, ddof=1),min(diff_fit),max(diff_fit)]
                    polyfit_o.append(order)
                    polyfit_p.append(p)
            polyfit_o = np.array(polyfit_o)
            polyfit_p = np.array(polyfit_p)
...
def adjust_wavelength_solution(params, spectrum, arc_lines_px, wavelength_solution_ori, wavelength_solution_arclines_ori, reference_catalog, reference_names, xlows, xhighs, show_res=False):
    ...
    for step in range(steps):
        ...
        for iteration in range(iter_break):
                ...
                weight = arc_lines_wavelength[:, 4]/np.median(arc_lines_wavelength[:,4])+arc_lines_wavelength[:,5]/np.median(arc_lines_wavelength[:,5])+arc_lines_wavelength[:,6]/np.median(arc_lines_wavelength[:,6])
                ...
                for i in range(arc_line_wave.shape[0]):
                    ...
                    diff_catalog = reference_catalog[:,0] - arc_line_wave[i]
                    ...
                    if min(abs(diff_catalog)) <= max_diff_px*arc_line_res[i]:   #calculate the approximate wavelength difference for max_diff_px
                        ...
                        arc_lines_wavelength.append([arc_lines_px[i,0], arc_lines_px[i,1], catalog_line[0], diff_catalog[pos_min], weight1, weight2, weight3, pos_min, arc_lines_px[i,2] ])
                arc_lines_wavelength = np.array(arc_lines_wavelength)   
                # arc_lines_wavelength: order, px in spectrum, wavelength in reference catalog, difference in wavelength between wavelength solution and reference line, 3x weights, index in refence catalog, width of the line
    ...
    diff_real_cent = np.abs(cenwave-np.polyval(p_real_cent, 1.0/(order_offset+orders)))
    ...
    # Log the results
    avg_diff_fit = np.mean(np.abs(arc_lines_wavelength[:,3]))
    std_diff_fit = np.std(np.abs(arc_lines_wavelength[:,3]), ddof=1)
    logger('Info: used {0} lines. The average deviation of the fit is {1} +/- {2} Angstrom (abs of the residuals). A 2d polynom fit with {3} orders in dispersion direction (along the traces) and {4} orders in cross-dispersion direction was used. With this solution, the offset to real orders is {5}. With this offset, the average deviation (abs) of the residuals between the central wavelengths and the grating equation is {6} +- {7} Angstrom. Using the original solution gives an offset of {8}.'.format(arc_lines_wavelength.shape[0], round(avg_diff_fit,4), round(std_diff_fit,4), polynom_order_trace, polynom_order_intertrace, order_offset, round(np.mean(diff_real_cent),4), round(np.std(diff_real_cent, ddof=1),4), order_offset_old ))
    if std_diff_fit > 10 or std_diff_fit < 1E-10:
    ...
    logger('Wavelenght solution in 2d (for pixel and order at the same time) is [No of orders1, No of orders2, central pixel, offset to real order, parameters of the 2d polynomial(px^0*m^0, px^1*m^0, px^0*m^1, px^2*m^0, px^1*m^1, px^0*m^2, ....)]: \n{0}'.format(wavelength_solution), show=False)
    ...
    t = 'Residuals of the identificated arc lines: identified {0} lines for a 2d polynom fit with {1} and {2} orders'.format(arc_lines_wavelength.shape[0], polynom_order_trace, polynom_order_intertrace)
    plot_img_spec.plot_points(x,y,l,[params['logging_arc_line_identification_residuals']],show=False, title=t, x_title='Pixel', y_title='Residual')
    ...
    for order in orders:
        ...
        #print [len(diff_fit), np.std(diff_fit, ddof=1),min(diff_fit),max(diff_fit)]
...
def statistics_arc_reference_lines(data, positions, reference_names, wavelength_solution, xlows, xhighs):
    ...
    logger('\t\torder\tcenwave\tminwave\tmaxwave\tranwave\tAng/px\tname\tnumber\tgausswidth_avg\tgausswidth_std\tmin_refline\tmax_refline\trange_reflines_whole_order',printarrayformat=printarrayformat, printarray=result)
...
def adjust_wavelength_solution2(params, spectrum, arc_lines_px, wavelength_solution_ori, wavelength_solution_arclines_ori, reference_catalog, reference_names, xlows, xhighs, show_res=False):
    """
    :return wavelength_solution_arclines: list with one entry for each order. Each order contains the wavelengths of the identified reference lines, sorted by the arc_lines_px (brightest to faintest)
    """
    ignoreorders = [] #[0,1,2,3,4,5,6,7,8]
    ...
    diff_real_cent = np.std(diff_real_cent, ddof=1)
    # Redo the 2d polynomial fit (it should also re-create the arc_lines_wavelength table, but most times the difference is 0)
    x = arc_lines_wavelength[:,1]-cen_px[0]
    y = 1.0/(arc_lines_wavelength[:,0]+order_offset)
    weight = arc_lines_wavelength[:, 4]/np.median(arc_lines_wavelength[:,4])+arc_lines_wavelength[:,5]/np.median(arc_lines_wavelength[:,5])+arc_lines_wavelength[:,6]/np.median(arc_lines_wavelength[:,6])
    ...
    avg_diff_fit = np.mean(np.abs(arc_lines_wavelength[:,3]))
    std_diff_fit = np.std(np.abs(arc_lines_wavelength[:,3]), ddof=1)
    logger('Info: used {0} lines. The average deviation of the fit is {1} +/- {2} Angstrom (abs of the residuals). A 2d polynom fit with {3} orders in dispersion direction (along the traces) and {4} orders in cross-dispersion direction was used. With this solution, the offset to real orders is {5}. With this offset, the standard deviation of the residuals between the central wavelengths and the grating equation is {6} Angstrom. Using the original solution gives an offset of {7}.'.format(arc_lines_wavelength.shape[0], round(avg_diff_fit,4), round(std_diff_fit,4), polynom_order_trace, polynom_order_intertrace, order_offset, round(diff_real_cent,4), order_offset_old ))
    if std_diff_fit > 10 or std_diff_fit < 1E-10:
        logger('Error: The wavelength solution seems wrong. Please check the parameters "order_offset", "px_offset", and "px_offset_order". It might be useful to compare the file with the emission lines ({0}) in the current folder and the folder folder with the previous wavelength solution (see parameter "original_master_arc_solution_filename")'.format(params['master_arc_l_filename']))
