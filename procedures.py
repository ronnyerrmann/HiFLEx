...
def polynomial_fit_2d_norm(xx, yy, zz, xord, yord, w=[], w_range=1E50):
    ...
    if ww.shape[0] > 0:
        if np.min(ww)/(np.max(ww)+0.0) < 0.5:
            ...
            ww = ww/(np.min(ww)+0.0)
            if np.max(ww) > w_range:
                ww = ww / ( np.max(ww) / (w_range+0.0) )             # scale by w_range
            mem = psutil.virtual_memory()                   # svmem(total=33221091328, available=28485840896, percent=14.3, used=4202041344, free=25513508864, active=..., inactive=..., buffers=..., cached=.., shared=...)
...
def sigmaclip(data, x=[], nc=1, sub=[], ll=3., lu=3., repeats=5, exclude_absorption=False, x_cen=np.nan):
    ...
    if len(data) == 0:
        ...
        return np.array([], dtype=bool), poly
    ...
    if len(data) > 1:
        if np.std(data) < 1E-10:
            return np.ones(len(data)).astype(bool), np.array([0]*nc+[data[0]])
    if np.isnan(x_cen):
...
def width_order(y, oldcenter, maxFWHM):
    ...
    linemax = np.max(y[max(0,int(round(oldcenter))-1):min(len(y),int(round(oldcenter))+2)])
    linemin = np.min(y[leftmin:rightmin+1])
    ...
    if len(rangediff) != 0 and len(range2pct) != 0:
        left = max(np.min(rangediff),np.min(range2pct))
        right = min(np.max(rangediff)+smoothdiff,np.max(range2pct))
    elif len(rangediff) != 0:
        left = np.min(rangediff)
        right = np.max(rangediff)+smoothdiff
    elif len(range2pct) != 0:
        left = np.min(range2pct)
        right = np.max(range2pct)
    #if right - left < 2:
...
def prepare_and_fit_gauss(x, y, center, width, significance=3):
    """
    only use the important things from centroid_order
    :params center: list with minimum and maximum
    """
    miny = np.min(y)
    if np.isnan(miny):
        logger('Programming error: Please make sure to only hand over non-nan values to prepare_and_fit_gauss')
    rangey= np.max(y) - miny
    p0=[rangey, np.median(center), width, miny]
    ...
    stdfit = np.std(oneD_gauss(x,popt)-y, ddof=popt.shape[0])           # if the average is 0, than this gives the stray of the residuals
    if stdlin/(stdfit+0.0) >= significance:# or ( popt[0]/(stdfit+0.0) >= significance and popt[1] > np.min(x) and popt[1] < np.max(x) ):   # The last bit includes too many non-lines for 20210419/Tung-0022_1p0s
        #print(round(stdlin), round(stdfit), significance, popt, np.min(x), np.max(x), y)
        return popt
    else:
        #print(round(stdlin), round(stdfit), popt, np.min(x), np.max(x), y)
        return np.array([0,0,0,0])
...
def centroid_order(x, y, center, width, significance=3, bordersub_fine=True, blended_gauss=False, bugfix=False):
    ...
    if abs(x[pos_max] - center) > width/2.:              # changed on 20180525 from (x[pos_max] - center) < width/2
        center = x[pos_max]
    p0=[rangey,center,width,miny]
    bounds=((0.2*rangey, center-max(1.2,0.3*width), 0.1*width, miny-0.2*rangey), (5.*rangey, center+max(1.2,0.3*width), 2.*width, miny+0.2*rangey))
    if blended_gauss:
        p0 = p0 + p0
    ...
    for border_sub in border_subs:    
        range_data = np.arange(border_sub[0],len(x)-border_sub[1])
        if len(range_data) <= 6:
            break
        if not blended_gauss:
            ...
            try:
                ...
                continue
            #get_timing('after ')
            stdfit = np.std(oneD_gauss(x[range_data],popt)-y[range_data], ddof=popt.shape[0])           # if the average is 0, than this gives the stray of the residuals
...
def find_border_pctl(data, border_pctl=50, full_px=False):
    ...
    data -= np.min(data)                                                   # Set min(data) to zero
    ran_pctl = np.max(data)*border_pctl/100.
    higher = np.where( data > ran_pctl)[0]                              # Find the indexes above the percentile value, can have gaps
    higher = [ max(1, np.min(higher)) , min(len(data)-2, np.max(higher)) ]    # To allow a 2px fit, exclude the outermost pixels; otherwise it contains the first and last index over border_pctl
    ...
    if len(lower2) < 1:                                                 # happens if the line is cut before it reaches the lower level
        lower2 = [len(data)-1]
    if full_px:
        return np.max(lower1), np.min(lower2)                                 # Returns full pixel to speed things up
    ran1 = np.arange( np.max(lower1), higher[0]+1 )                      # This should be a 2px (or more) transition around the transition
    ran2 = np.arange( higher[1], np.min(lower2)+1 )                      # This should be a 2px (or more) transition around the transition
    result = []
    for ran in [ran1, ran2]:
        if np.std(data[ran], ddof=1) == 0.0:                            # same flux values in all pixel -> fit is not possible
            ...
            if np.min(ran)-1 <= fit_pos <= np.max(ran)+1:
...
def find_center(imslice, oldcenter, x, maxFWHM, border_pctl=0, border_pctl_poly=75, significance=3.0, polymax=False, bugfix=False, dummy=[]):
    ...
    :param x: integer, index of the 1D array in the overall image, only used for error investigation
    ...
    :param polymax: bool, fits the centre also with a 3rd order polynomial to find the maximum in this
...
def create_shifted_array(input_list, shifts, keep_length=True, end_to_beginning=False, replace_undefined=np.nan):
    """
    ...
    :param replace_undefined: replace values when end_to_beginning==False with this value
...
def measure_noise(spectra, p_order=16, semi_window=10, high_noise=None, minimum_SNR=None):
    ...
    for order in range(specs[0]):
        ...
        shift_res = create_shifted_array(res, np.arange(-semi_window, semi_window+1))        # one line for each shifted spectrum
        ...
        for dummy in range(2):          # Smooth out: remove high noise where it's only an island of a few pixel
            ...
            shift_xarr = create_shifted_array(xarr, np.arange(semi_window, -semi_window-1, -1))        # one line for each shifted spectrum
            ...
            # which values to use: nonnan == True
            shift_nonnan = nonnan[shift_xarr]
            shift_nonnan[~shift_xarr_defined] = False
            sum_nonnan = np.sum(shift_nonnan, axis=0)                # How many values were nonnan for this pixel
            # high noise values
            shift_high_noise = high_noise_array[order,:][shift_xarr]                    # 1D_array will be extendet to shape of shift_xarr
            shift_high_noise[~shift_xarr_defined] = False
            sum_high_noise = np.sum(shift_high_noise, axis=0)      # How many values are high noise for each pixel
            # Is the noise too high in the area -> exclude more points
            to_1_bool = (sum_high_noise >= sum_nonnan )
            # extend to semwindow and ignore all that are False
            shift_to_1_bool = create_shifted_array(to_1_bool, np.arange(semi_window, -semi_window-1, -1), replace_undefined=False)
            to_1 = np.sum(shift_to_1_bool, axis=0)
            shift_to_0_bool = create_shifted_array(~to_1_bool, np.arange(semi_window, -semi_window-1, -1), replace_undefined=False)
            to_0 = np.sum(shift_to_0_bool, axis=0)
            """
            #get_timing('x3 ')
            to_1 = np.zeros(specs[1])
            to_0 = np.zeros(specs[1])
            for index_f in xarr[nonnan]:        # This still takes 0.13 seconds at 6000px; !!! Ignore areas that don't contain any to_1_bool in +- 2*semi_window
                x_s = shift_xarr[shift_xarr_defined[:,index_f],index_f]
                #x_s = xarr[max(0,index_f-semi_window):min(specs[1],index_f+semi_window) + 1 ]
                #sub = high_noise_array[order, x_s]
                #if np.sum(sub) != sum_high_noise[index_f]:
                #    print( order, index_f, np.sum(nonnan[x_s]), sum_nonnan[index_f] )
                if to_1_bool[index_f]:      # not sub.shape[0], as this includes nan values at the borders
                    to_1[x_s] += 1
                else:
                    to_0[x_s] += 1
                #get_timing('x5 ')
...
def estimate_width(im):
    ...
    for i in range(maxtests):     # Use at least 200 positions in order to be sure to find at least 10
        ...
        if len(widths) >= 50:
            break
    if len(widths) < 10:
        logger('Warn: Width of the traces/emission line features could not be determined, only {0} traces/lines found when testing {1} positions. Assuming 1 pixel as Gaussian width.'.format(len(widths), maxtests ))
        return 1.
...
def adjust_trace_order(kwargs):
    ...
    if True:
        ...
        xlows_o = max(0,np.min(positions[:,0])*binx-5)
        xhighs_o = min(imus[0],(np.max(positions[:,0])+1)*binx+5)
...
def extract_order(kwarg):
    ...
    if w_mult == 0.0:                                                       # only the central full pixel
        ...
        if var == 'linfit':
            #lowersb[lowersb<0] = 0                                              # If the order is at the boundary, only extract until the boundary
            #uppersb[uppersb>maxpx-minpxo] = maxpx-minpxo                        # If the order is at the boundary, only extract until the boundary
            ...
            for xrow in xarr1[order_in_frame]:          # A bit faster, but the loop is still there, hence only 30% faster
                ...
                if uppers[xrow] - lowers[xrow] < 1.0:       # There will be no full pixel, which messes up everything
                    ...
                    fracpixparam = []
                    if lowersb[xrow] >= 0:
                        fracpixparam.append([lowers[xrow], 'l', 1])
                    if uppersb[xrow] <= maxpx-minpxo:
                        fracpixparam.append([uppers[xrow], 'u', 1])
                ...
                for [borderpos, pos, number_px] in fracpixparam:
                    ...
                    if x.shape[0] <= 1: # keep things as they are
                        continue
                    y = image[xrow, x]
                    ...
                    if pos == 'l':
                        ...
                        #print(x.shape,image_sub.shape, xrow, uppersb.shape, uppers.shape, uppersb_0p5_minpxo.shape, uppersb[xrow], uppers[xrow], uppersb_0p5_minpxo[xrow])
...
def shift_orders(im, params, sci_tr_poly, xlows, xhighs, oldwidths, in_shift = 0):
    ...
    if len(problem_order) > 0:
        ...
        if len(problem_order) > 0.5*sci_tr_poly.shape[0]:
            ...
            shift_error = -1
    logger('Info: The shift of the traces was determined to {0} +- {2}, including the input-shift of {1}.{3}'.format(round(shift+in_shift,2), in_shift, round(shift_error,2), problem_text ))
...
def find_shift_images(params, im, im_ref, sci_traces, w_mult, cal_tr_poly, extract=True, im_head=dict(), max_allowed=[-100,100]):
    ...
    if shift is None:
        ...
        for start_order in range(int(sci_tr_poly.shape[0]/3)):             # If orders are to close in the red, then no shift can be determined -> might ignore the first third of the red orders
            shifts = []
            if sci_tr_poly.shape[0] == 1:   # only one order, e.g. when using live_extraction
                shift = 10 * np.max(widths[:,2])
            else:                           # normal case
                ...
                shift = np.min(shifts)
            shift = min(shift, 20 * np.max(widths[:,2]))
            if shift >= 3:              
                break
        ...
        for shift in range(max(np.abs(range_shifts))+1):
            for pm in [-1, +1]:
                if extract:         # extract and find the maximum flux
                    ...
                    ims2 = diffim.shape[1]
                    temp = np.max(np.abs(range_shifts).astype(int))-abs(shift)
        ...
        if popt[2] > 0.5 * (max(range_shifts) - min(range_shifts)) or popt[1] < min(range_shifts) or popt[1] > max(range_shifts) or abs(popt[1]) > params['extraction_shift']:
            ...
            for dummy in range(5):                                                                  # repeat several times in order to get closer and closer to the maximum
                ...
                fshifts = np.linspace( np.min(best_shifts), np.max(best_shifts), 7)
            ...
            if not np.isnan(shift_poly):
                shift = shift_poly
            elif width == 0 and np.min(shifts) < mshift < np.max(shifts):      # If Gaussian fit didn't work, or if shift is close to borders, then mshift might be dogy
                shift = mshift                                      # use the maximum flux as shift
        min_shifts, max_shifts = np.min(shifts), np.max(shifts)
...
def arc_shift(params, im, pfits, xlows, xhighs, widths):
    ...
    logger(('Info: The shift between science traces and calibration orders is between {0} and {1} px. '+\
            ...
            'The Gaussian width of the arc lines in spacial direction is between {2} and {3} px').format(np.min(shifts), np.max(shifts), 
...
def identify_emission_lines_single_order(kwargs):
        ...
        for i in range(len(small_yarr)):
            ...
            small_yarr[pos_max] = np.min(small_yarr)-1     #do not find the line again
            ...
            if True:       # old way until v1.0.0 (including), re-introduced shortly after, at least with version 3.3.4
...
def read_reference_catalog(params, filenames=None, wavelength_muliplier=None, arc_lines=None):
    ...
    :param wavelength_muliplier: float, if the resolution is not given in Angstroms, than it can be converted into Angstroms with this factor
...
def shift_wavelength_solution(params, aspectra, wave_sol_dict, reference_lines_dict, xlows, xhighs, obsdate_float, jd_midexp, sci_tr_poly, cal_tr_poly, objname, maxshift=20.0, in_shift=0, fib='cal', fine=False, im_head=dict()):
    ...
    if np.max(wavelength_solution[:,-1]) < 1000:                 # pseudo solution, wavelength of the central pixel is < 1000 Angstroms
    ...
    if 'linestat' in wave_sol_dict.keys():
        FWHM = wave_sol_dict['linestat']*2.35482            # per order: [median, std]
        med_FWHM = np.median(FWHM[:,0])
    else:
        med_FWHM = max(1, int(round(estimate_width(aspectra)*2.35482)))
        FWHM = np.array([[med_FWHM,0]]*wavelength_solution.shape[0])       # maybe seach for width instead?
    FWHM = np.vstack(( FWHM.T, FWHM[:,0]*1.5/2.35482 )).T              # To exclude too wide Gaussian fits later
    ratio_lines_identified = 0.15       # if more than ratio_lines_identified of the checked_arc_lines has been identified, then a sigma_clipping will be applied. If less than this number of lines remain after sigma clipping, then the assumption is, that the calibration fiber wasn't used and therefore no wavelength shift is applied
    ...
    orders = np.arange(wavelength_solution.shape[0])
    good_orders = ( (FWHM[:,0] > 0) & (wavelength_solution_arclines[:,0] > 0) )     # No emission line was identified, or no line from catalogue was identified
    
    good_lines = (wavelength_solution_arclines > 0)
    number_lines = np.sum(good_lines, axis=None)
    data = np.zeros((number_lines,11))
    data[:,0] = np.arange(number_lines)
    # Numbering [0], Order [1], Wavelength from solution [2], px from wavelength, corrected by in_shift [3], rounded px3 [4], found px [5], shift [6], width [7], when was it last fitted [8], index reference catalogue [9], wavelength reference catalogue [10]
    wavelength_array = np.zeros(ass)
    index = 0
    for order in orders:
        number_good_data = np.sum(good_lines[order,:])
        range_data = np.arange(index, index + number_good_data)
        wavelength_array[order,:] = np.polyval(wavelength_solution[order,2:], xarr-wavelength_solution[order,1])     # Wavelength of each pixel in the order
        # Get the line from the reference catalogue
        indexes = np.arange(wavelength_solution_arclines.shape[1])
        diff_2darray = np.abs( np.tile(reference_catalog[:,0], (number_good_data,1)) - np.tile(wavelength_solution_arclines[order][good_lines[order,:]],(reference_catalog.shape[0],1)).T )
        ref_line_indexes = np.argmin(diff_2darray, axis=1)
        min_diff = np.min(diff_2darray, axis=1)     # Best matching reference line for each identified line
        ref_wave = reference_catalog[ref_line_indexes,0]
        bad_data = (min_diff > 0.05)                # Only use lines which are within 0.05 Angstom in the catalogue -> is that is important?
        ref_line_indexes = ref_line_indexes.astype(float)
        ref_line_indexes[bad_data] = np.nan  # check that it is in the reference catalog, allowing for uncertainty; catches also zeros used to fill the array
        ref_wave[bad_data] = np.nan
        data[range_data,9] = ref_line_indexes
        data[range_data,10] = ref_wave
        #print(order,range_data.shape, ref_line_indexes.shape, ref_line_indexes)
        
        data[range_data,1] = order          # Order
        data[range_data,2] = wavelength_solution_arclines[order,good_lines[order,:]]    # Wavelength from the solution
        for ii in range_data:
            pos_fine = invert_polyval(wavelength_solution[order,2:], xarr, data[ii,2], poly0=wavelength_solution[order,1], prec=1E-2) # position of arcline in xarr
            data[ii,3] = pos_fine        # Pixel for the wavelength
    ...
    data[:,6] = 5*maxshift                  # Will become the shift
    data[:,8] = -10                         # So it fits already in the first round of the loop
    shift_range = np.arange(int(maxshift)+1, -int(maxshift)-2, -1)          # reverse, so px are increasing
    shift_med, shift_std = in_shift, maxshift
    fine, last_round = False, False
    """ data:
    0: index
    1: order
    ...
    """
    max_number_repeat = int(2*maxshift)
    max_repeats_each_step = 5
    results = np.zeros((max_number_repeat*max_repeats_each_step,8))*np.nan       # results from each step:
    ...
    for number_repeat in range(max_number_repeat):
        ...
        # data_shifts = shift_xarr[ data[:,3], :]
        # data_shifts_defined = shift_xarr_defined[ data[:,3], :]
        # But what do i do with it now
        
        # Use only values outside the area, if not fit in the last two rounds, and if not excluded forever
        replace_values = ( ( data[:,6] > np.max(shift_range) ) | ( data[:,6] < np.min(shift_range) ) ) & (data[:,5] < 1E6) & (data[:,8] < number_repeat-3)  # outside values only every 3 rounds
        if last_round:
            replace_values = ( ( (data[:,6] > shift_med + sigma*shift_std ) | (data[:,6] < shift_med - sigma*shift_std ) )  & (data[:,8] < number_repeat-1) )  | replace_values     # inside values, but a bit off, only if not last round
        data[replace_values,8] = number_repeat
        for ii in data[replace_values,0].astype(int):
            order = int(data[ii,1])
            xpos = int(data[ii,4])                                  # rounded value of xpos
            xx = shift_xarr[shift_xarr_defined[:,xpos],xpos]          # x-data to fit
            yy = aspectra[order,xx.astype(int)]                                             # spectrum
            notnan = ~np.isnan(yy)
            if np.sum(notnan) < 5:
                data[ii,5] == 2E6                                       # Exclude forever
                #print('nan', data[ii,:], np.sum(np.isnan(yy)), yy.shape, np.min(xx), np.max(xx))
                continue
            # This takes long, 0.02s, but for 3000 lines that are 60 seconds
            ...
            if fine:                    # if xx is int then popt center will be int?
                ...
                #if order == 6 or order == 36:
                print(order, data[ii,2], data[ii,3], popt, popt2, nanminmax(xx[notnan]), nanminmax(xarr[range_arr]) )
                """
                if popt[1] == 0 or popt[2] > FWHM[order,2]:     # Bad fit
                    data[ii,5] = -1E3
                    #print(number_repeat, ii, popt, xx, yy)
                    #if ii > 100: exit(100)
                else:
                    data[ii,5] = popt[1]
                    data[ii,7] = popt[2]                # Gaussian width
            else:
                posmax = np.nanargmax(yy[notnan])
                posmax = xx[notnan][posmax]
                data[ii,5] = posmax
            #if ii%100 == 0:
            #if ii in [18,55,120,142,184,204,207,229,238]:
            #    print(fine, number_repeat, ii, order, xpos, data[ii,5], xx, yy)
        data[:,6] = data[:,5] - data[:,3]               # Shifts
        for jj in range(max_repeats_each_step):
            good_values = (data[:,5] > 0) & (data[:,5] < 1E6) & (data[:,6] >= np.min(shift_range)) & (data[:,6] <= np.max(shift_range))
            data_sub = data[good_values]
            if data_sub.shape[0] < 10:
                if jj == 0:         # only do in first round
                    shift_med, shift_std = in_shift, 0
                    width_avg, width_std = 0, 0
                    logtext = 'No lines identified.'
                break
            shift_med_all, shift_std_all = np.median( data_sub[:,6] ), np.std( data_sub[:,6], ddof=1)     # Median of all positions
            width_avg, width_std = np.mean(data[good_values,7]), np.std(data[good_values,7], ddof=1)
            logtext = 'Median and Standard deviation of the offset of all useful {0} refitted lines (from the wavelength solution) is {1} +- {2} px.'.format(data_sub.shape[0], round(shift_med_all,4), round(shift_std_all,4))
            if data_sub.shape[0] < ratio_lines_identified * data.shape[0]:  # not enough lines
                if jj == 0:         # Only do in first round, otherwise use the values from the loop before
                    shift_med, shift_std = shift_med_all, shift_std_all
                break
            min_shift = np.min(data_sub[:,6])
            max_shift = np.max(data_sub[:,6])
            # Get an average shift
            
            bin_step_w = np.hstack(( np.arange(0.4, max(1,(max_shift-min_shift)/5.), 0.1),
                                     np.arange(0.2, 0.4, 0.05), np.arange(0.1, 0.2, 0.02) ))      # width of bins, 
            test_bins = np.unique( np.round((max_shift-min_shift)/bin_step_w).astype(int) )       # translate into number of bins
            test_bins = test_bins[test_bins>=6]
            ii = 6
            while test_bins.shape[0] < 6:
                test_bins = np.unique( np.append(test_bins, ii) )
                ii += 1
            # !!! Loop around and remove outliers
            bin_results = np.zeros((test_bins.shape[0],5))
            for ii, bins in enumerate(test_bins):
                hist_y, hist_x = np.histogram( data_sub[:,6], bins )        # Look at the results in a histogram 
                hist_x2 = (hist_x[1:]+hist_x[:-1])/2.                       # center of the bin, same length as hist_y
                posmax = np.argmax(hist_y)
                bin_results[ii,0] = hist_x2[posmax]                         # Which shift has the highest occurence
                popt = centroid_order(hist_x2, hist_y, hist_x2[posmax], (hist_x2[-1]-hist_x2[0])*0.5, significance=2.5, bordersub_fine=False)    #a,x0,sigma,b in a*np.exp(-(x-x0)**2/(2*sigma**2))+b
                bin_results[ii,1:] = popt                                   # Fit a Gaussian, that is probably the best fit
                #print(bins, bin_results[ii,:], round(np.mean(hist_x[1:]-hist_x[:-1]),4), max_shift, min_shift, hist_y )
            
            shift_med_bin = np.average( bin_results[:,0], weights=test_bins )       # Highest bin
            shift_std_bin = max(np.std( bin_results[:,0], ddof=1 ), np.max(hist_x[1:]-hist_x[:-1]))    # Max from STD or at least the largest step
            index = number_repeat*max_repeats_each_step+jj
            results[index,0] = number_repeat
            results[index,1] = jj
            results[index,2] = fine
            results[index,3] = data_sub.shape[0]
            results[index,4] = shift_med_bin
            results[index,5] = shift_std_bin
            logtext += ' When binning the data the best fit is {0} +- {1} px.'.format(round(shift_med_bin,4), round(shift_std_bin,4))
            shift_med, shift_std = np.mean([shift_med_all, shift_med_bin]), np.mean([shift_std_all, shift_std_bin])
            
            good_popts = ( bin_results[:,3] != 0 )
            if np.sum(good_popts) > 3:                                  # Best Gaussian fits
                popt_med = np.median(bin_results[good_popts,1:], axis=0)
                shift_med_gauss, shift_std_gauss = popt_med[1], popt_med[2]         # Gauss fitted
                results[index,6] = shift_med_gauss
                results[index,7] = shift_std_gauss
                logtext += (' The Gaussian centre and width of the distribution of the refitted lines gives: {0} +- {1} px '+\
                            '(the values depend on the binning, standard deviation between {2} different bins is {3} px).').format(round(shift_med_gauss,4), 
                                                                round(shift_std_gauss,4), np.sum(good_popts), round_sig(np.std(bin_results[good_popts,2], ddof=1),3))
                if (abs(shift_med - shift_med_gauss) > sigma*shift_std_gauss) or  (shift_std_gauss > 2*shift_std_bin):      # The Gauss is not trustworthy
                    logtext += ' Used the mean of the previous values, excluding the Gauss.'
                else:
                    shift_med, shift_std = shift_med_gauss, shift_std_gauss
                    logtext += ' Used the values of the Gauss.'
            else:
                logtext += ' Used the mean of the previous values.'

            # Exclude the outliers
            replace_values = good_values & ( ( data[:,6] > shift_med + max(sigma*shift_std,1E-3) ) | (data[:,6] < shift_med - max(sigma*shift_std,1E-3) ) )     # at least one milli-pixel uncertainty
            #print(number_repeat, jj, fine, nanminmax(shift_range), med_FWHM, np.sum(good_values), np.sum(replace_values), data.shape, data_sub.shape, round(min_shift,4), round(max_shift,4), np.median(FWHM,axis=0), logtext, hist_y, hist_x2 )      #np.vstack((test_bins, bin_results[:,[0,2,3]].T)).T,
            if not replace_values.any():        # No more changes
                break
            data[replace_values,6] = 5*maxshift
            data[replace_values,5] = -1E-3

        # Recreate shift_range, making sure things only move by one pixel
        prev_shift_range = nanminmax(shift_range)
        smaller, larger = np.sum(shift_range<shift_med), np.sum(shift_range>shift_med)
        if smaller > larger:        # shift_med is positive -> larger values
            start, end = prev_shift_range[0]+2, prev_shift_range[1]+1
        elif smaller < larger:
            start, end = prev_shift_range[0]-1, prev_shift_range[1]-2
        else:
            start, end = prev_shift_range[0]+1, prev_shift_range[1]-1
        #shift_range = np.unique( np.arange(end, start-1, -1) )      # remove 2 pixel for next round; "+shift_med" tested on data with shift of -2.5px
        #shift_range[::-1].sort()        # np.unique sorts the values, this reverses them again
        shift_range = np.arange(end, start-1, -1)
        if ( (shift_range.shape[0] <= 20*shift_std and shift_std > 0 ) or shift_range.shape[0] <= 15 or number_repeat+5 > max_number_repeat or shift_range.shape[0] < med_FWHM*2) and not fine:        # 20*shift_std tested on <0.01 Ang/px camera
            fine = True
            data[:,5] = -1E-3
            data[:,6] = 5*maxshift                  # Will become the shift
            data[:,8] = number_repeat - 5
            #print('Switch to fine')
        if shift_range.shape[0] <= 6 or data_sub.shape[0] == 0 or (shift_range.shape[0] < med_FWHM*3 and last_round):            # Not enough data points
            break
        elif shift_range.shape[0] <= 8 or shift_range.shape[0] < med_FWHM*2:
            last_round = True
            #print('last round')
        elif last_round:                        # Switch off again to not rerun everything again and again
            last_round = False
        #print(shift_range, smaller, larger, start, end, prev_shift_range, shift_med)
        
    # [0] = number_repeat, [1] = repeat in the step, [2] = fine, [3] = good data points, [4] = shift_med_bin, [5] = shift_std_bin, [6] = shift_med_gauss, [7] = shift_std_gauss
    good_res = (results[:,2] == fine) & (results[:,3] > data_sub.shape[0])
    good_res1 = good_res & (results[:,5] < 1.3*shift_std)
    good_res2 = good_res & (results[:,7] < 1.3*shift_std)
    shifts = np.hstack(( results[good_res1,4], results[good_res2,6] ))
    lines = np.hstack(( results[good_res1,3], results[good_res2,3] ))
    if lines.shape[0] > 0:
        extra_text = ' When allowing 30% more uncertainty the shifts are between {0} and {1}, using up to {2} lines.'.format(round(np.min(shifts),3), round(np.max(shifts),3), int(np.max(lines)) )
    else:
        extra_text = ''
    logger('Info: '+logtext+' The resulting shift between {2}-fiber and the emission line spectrum in {3} of {0} +- {1} px will be used.'.format(round(shift_med,4), round(shift_std,4), fib, objname)+
            extra_text )
    wavelength_solution_new = copy.deepcopy(wavelength_solution)
    if data_sub.shape[0] >= 0.5 * ratio_lines_identified * data.shape[0]:                          # Only if enough lines have been detected
        if params['log_wavesolshift_details']:
            logger('Step: plotting results of wavelength shift.')
            # Log the results
            ...
            bisector_measurements_emission_lines(fname, aspectra, data_sub, [1,5,7])         # order, pixel, gauss width
            
            y_title='Residual (O-C) [Pixel]'
            x, w, y, l = [], [], [], []
            for order in range(ass[0]):
                inorder = ( data_sub[:,1]==order )
                x.append(data_sub[inorder,5])    # Pixel
                w.append(data_sub[inorder,2])    # Wavelength
                y.append(data_sub[inorder,6])    # Residuals
                l.append('{0}'.format(order))
            text = 'Residuals to the wavelength solution: used {0} out of {1} lines'.format(data_sub.shape[0], data.shape[0])
    ...
    logtext2 = 'The overall shift between wavelength solution and the current calibration spectrum'
    if not params['extract_wavecal']:         # science and calibration traces are at the same position and it's not the calibration spectrum
        ...
        if fib == 'cal':
            ...
            logtext2 = 'The overall shift to be applied to the wavelength solution in the science fiber for the current calibration spectrum'
    ...
    logger(('Info: '+logtext2+' of file {9} at JD {10} is {0} +- {1} px ({8} km/s). {2} reference lines have been used, {7} reference lines have been tested.'+\
            ' The calibration lines have a Gaussian width of {3} +- {4} px, which corresponds to a FWHM of {5} +- {6} px.').format(round(shift_med,4), round(shift_std,4), 
                        data_sub.shape[0], round(width_avg,3), round(width_std,3), round(width_avg*2.35482,3), round(width_std*2.35482,3), data.shape[0],
    ...
    im_head['HIERARCH HiFLEx D_SHIFT_NUMBER_LINES'] = (data_sub.shape[0], 'out of {0} calibration lines'.format(data.shape[0]))
    im_head['HIERARCH HiFLEx D_WIDTH'] = (round(width_avg,2), 'Gaussian width of the calibration lines [px]')
    im_head['HIERARCH HiFLEx D_WIDTH_ERR'] = (round(width_std,3), 'Uncertainty of the width [px]')
    im_head['HIERARCH HiFLEx D_SHIFT_KMS'] = (d_shift_kms, 'Offset in dispersion direction [km/s]')
    if data_sub.shape[0] >= ratio_lines_identified * data.shape[0] and data_sub.shape[0] != 0:                          # Statistics only if enough lines were detected
        statistics_arc_reference_lines(data_sub, [1,9,7,10], reference_names, wavelength_solution, xlows, xhighs, show=False)        # Order, index in reflines catalogue, Gaussian width, wavelength in reference catalogue
...
def shift_wavelength_solution_times(params, wavelength_solution, obsdate_float, jd_midexp, objname, im_head):
    ...
    if not params['two_solutions']:                                 # single soution: use all lines
        ...
        if data_bef.shape[0] == 0:
            ...
            text = 'Median of the data points in {0} after the exposure ( {1} to {2} ).'.format( params['master_wavelengths_shift_filename'], np.min(data_aft[:,0]), np.max(data_aft[:,0]) )
        elif data_aft.shape[0] == 0:
            shift_avg = np.median(data_bef[:,1])
            text = 'Median of the data points in {0} before the exposure ( {1} to {2} ).'.format( params['master_wavelengths_shift_filename'], np.min(data_bef[:,0]), np.max(data_bef[:,0]) )
        else:
            data = np.vstack(( data_bef, data_aft ))
            p = np.polyfit(data[:,0], data[:,1], 1)
            shift_avg = np.polyval(p, jd_midexp)
            text = 'Linear fit of the data points in {0} around the exposure ( {1} to {2} ).'.format( params['master_wavelengths_shift_filename'], np.min(data[:,0]), np.max(data[:,0]) )
    ...
    if True:
        ...
        if 'cal' in modes and 'sci' in modes:
            ...
            text = 'It is the difference between science ({1}) and calibration ({2}) data points in {0}.'.format( params['master_wavelengths_shift_filename'], data['sci_text'], data['cal_text'] )
        else:       # only cal data
            shift_avg = 0 - data['cal_jd']
            text = 'It is the difference between calibration ({1}) data points in {0}.'.format( params['master_wavelengths_shift_filename'], data['cal_text'] )
...
def create_wavelengths_from_solution(params, wavelength_solution, spectra, wave_sols_sci=None, jd_mid=0, shift=0):
    ...
    :param jd_mid: float, JD of the middle of the exposure to create the wavelength array for
    ...
    if wave_sols_sci is None:           # Original, no interpolation of different solutions
        ...
        logger('Info: Used the following wavelength solutions of the science fibers and their corresponding weights to'+\
               ' calculate the wavelength array for JD (middle of the exposure) of {0} days{1}: '.format(round(jd_mid,4), textshift), show=False)
...
def invert_polyval(poly, xx, yy, poly0=0, prec=0.05):
    """
    ...
    xx_sub = np.arange(xx[max(0,index-3)], xx[min(xx.shape[0]-1,index+3)], prec)        # just get values around the best fitting area
    ...
    xx_inv = np.polyval(copy.deepcopy(poly_inv), yy)
    """stack = np.vstack((xx_sub, yfit_sub, np.polyval(poly_inv, yfit_sub) )).T
    pos = np.argmin(np.abs(stack[:,1] - yy))
    print( stack[pos-5:pos+6,:], poly_inv, yy, xx_inv, type(xx_sub), type(yfit_sub), type(poly_inv),type(xx_sub[0]), type(yfit_sub[0]), type(poly_inv[0]),  type(yy), type(xx_inv) )"""
...
def extraction_steps(params, im, im_name, im_head, sci_traces, cal_traces, wave_sol_dict_cal, reference_lines_dict, flat_spec_norm, im_trace):
    ...
    if 'HiFLEx BCKNOISE' not in im_head.keys():        # if not already done because background is in parameters
        # Due to binning the image it takes a few seconds
    ...
    if params['wavelength_scale_resolution'] > 0.0 and np.max(wave_sol_dict_cal['wavesol'][:,-1]) > 100:
        ...
        #wavelengths_lin, spectrum_lin = linearise_wavelength_spec(params, wavelength_solution_shift, spectra, method='sum', weight=espectra)
        #save_multispec([wavelengths_lin,spectrum_lin], params['path_extraction_single']+im_name+'_lin', im_head)
        if True:
        ...
        wavelengths_lin, spectrum_lin = linearise_wavelength_spec( params, wavearray, cspectra, method='weight', weight=((flat_spec_norm[index]+0.0)/espectra)**2 ) # highest throughput and lowest error should have the highest weight
        save_multispec([wavelengths_lin,spectrum_lin], params['path_extraction_single']+im_name+'_lin_cont', im_head)
        save_linspec_csv(params['path_extraction_single']+im_name+'_lin_cont.csv', wavelengths_lin, spectrum_lin)
        
    # For easier plotting
    add_text_to_file(params['path_extraction']+im_name+'.fits', params['result_path']+'plot_files.lst')
    add_text_to_file(obnames[0], params['result_path']+'objects_reduced.lst')
...
def save_linspec_csv(fname, wave, spec):
    with open(fname, 'w') as file:
            ...
            for ii in range(wave.shape[0]):
                if ~np.isnan(wave[ii]) and ~np.isnan(spec[ii]):
                    file.write('{1},{2}{0}'.format(os.linesep, round(wave[ii],4), round(spec[ii],5) ))
...
def scale_data(y, x=[], mode='gauss'):
    ...
    if len(x) != y.shape[0]:
        ...
        x = np.array(x)
    maxy = np.max(y)               # Continuum
    miny = np.min(y)               # could be line
    rangey = maxy - miny
    popt = [0,0,0,0]
    if mode == 'gauss':
        # Try to get the minimum of the line with a fit (if undersampled)
        center = int(y.shape[0]/2)
        width = np.max(x)-np.min(x)
        p0 = [rangey, x[center], 0.1*width, np.min(-y)]        # -y to make a positive Gauss
...
def calculate_bisector_multiple(data, data_x):
    ...
    :return bisecs: 2D array (x,y as rows): All data points from the bisector mesurements, sorted by y
...
def bisector_measurements_orders(im, fname, pfits, xlows, xhighs, widths=[]):
    ...
    xmin = np.min(xlows[ np.array(orders_to_check).flatten() ])
    xmax = np.max(xhighs[ np.array(orders_to_check).flatten() ])
...
def plot_wavelength_solution_form(fname, traces_def, wavelength_solution):
    ...
    colorbar = True  
    title = 'Wavelength difference between every {0} pixel (Angstroms/{0}px)'.format(step)
    axis_name = ['Order', 'Position in Dispersion direction [{0} px]'.format(step), 'wavelength difference [Angstroms/{0}px]'.format(step)]
    plot_img_spec.plot_image(im.T, [fname], pctile=0, show=False, adjust=[0.05,0.95,0.95,0.05], title=title, autotranspose=False, colorbar=colorbar, axis_name=axis_name, size=[16,10])
...
def plot_wavelength_solution_width_emmission_lines(fname, specs, data, indexes, step=20, title='Gaussian width'):
    ...
    title_f = '{1} every {0} pixel (white shows areas with no data available)'.format(step, title)
...
def plot_wavelength_solution_image(im, fname, traces_def, wavelength_solution, wavelength_solution_arclines, reference_catalog_full):
    ...
    for pp in orders:
            ...
            min_wave, max_wave = np.min(xarr_wave), np.max(xarr_wave)
            for source in [1,2]:
                if source == 1:
...
def transform_wavelength_solution_2d_to_n1d(number_orders, number_pixel, polynom_order_trace, polynom_order_intertrace, poly2d_params, order_offset, cen_px, arc_lines_wavelength):
    ...
    :return wavelength_solution: 2D array with number of orders and [real order, cen pixel, np.polyfit-parameters
    :return wavelength_solution_arclines: 2D array with number of orders and wavelengths of identified lines; filled with 0 to make array
    :return line_stats: 2D array with number of orders and [median, std] of line_width
    """
    ...
    for order in range(number_orders):
        ...
        inorder = (arc_lines_wavelength[:,0]==order)
        if np.sum(inorder) > 0:
            line_width = arc_lines_wavelength[inorder,9]                        # Gauss Width of the line
...
def adjust_wavelength_solution(params, spectrum, arc_lines_px, wavelength_solution_ori, wavelength_solution_arclines_ori, reference_lines_dict, traces_def, show_res=False, search_order_offset=False):
    ...
    if params['px_offset_order'][2] == 0.0:
        pxdiffords = np.zeros(1)
    else:
    ...
    for step in tqdm(range(steps+1), desc='Finding the new wavelength solution'):
        ...
        for iteration in range(iter_break):
                ...
                weight=np.zeros(0)      # cancel all above as weight isn't really helpful
                ...
                arc_line_wave = np.zeros(( arc_lines_px_step.shape[0], len(orders_use)+1 ))*np.nan
                arc_line_res  = np.zeros(( arc_lines_px_step.shape[0], len(orders_use)+1 ))*np.nan
                this_order    = np.zeros(( arc_lines_px_step.shape[0], len(orders_use) ))*np.nan
                res_cen       = np.zeros(( orders.shape[0], len(orders_use)+1 ))*np.nan
                ...
                for index_u, order_use in enumerate(orders_use):
                    ...
                    #print(orders_prev, order_use, np.unique( arc_lines_wavelength[inorder,0] ) )
                    ...
                    if old_px_range - px_range > 10:    # only use linear solution when more than 10 px more
                        ...
                        good_values_l = ( arc_lines_px_step[inorder2,1] <= old_px_range+pxdiff + min(100,4*(old_px_range - px_range)) ) & \
                                        ( arc_lines_px_step[inorder2,1] >= old_px_range+pxdiff) & ( ~np.isnan(x2[inorder2]) ) & ( ~np.isnan(y2[inorder2]) )                # lines on the left, covered by the old solution
                        good_values_r = ( arc_lines_px_step[inorder2,1] >= specs[1]-old_px_range+pxdiff - min(100,4*(old_px_range - px_range)) ) & \
                                        ( arc_lines_px_step[inorder2,1] <= specs[1]-old_px_range+pxdiff ) & ( ~np.isnan(x2[inorder2]) ) & ( ~np.isnan(y2[inorder2]) )      # lines on the right, covered by the old solution
                        ...
                        if np.sum(good_values_l) > 10:
                            poly2d_params_l = polynomial_fit_2d_norm(x2[inorder2][good_values_l], y2[inorder2][good_values_l], arc_line_wave_io[good_values_l], 1, polynom_order_intertrace_u)    # linear fit in the outsides of the order
                            good_values_l = ( arc_lines_px_step[inorder2,1] < old_px_range+pxdiff )   # lines on the left, only covered by the new solution
                            #print(np.vstack([ arc_lines_px_step[good_values_l,1], arc_lines_px_step[good_values_l,0], arc_line_wave_io[good_values_l], 
                            #                  arc_line_wave_io[good_values_l] - polynomial_value_2d(x2[good_values_l], y2[good_values_l], 1, polynom_order_intertrace_u, poly2d_params_l) ]).T)
                            arc_line_wave_io[good_values_l] = polynomial_value_2d(x2[inorder2][good_values_l], y2[inorder2][good_values_l], 1, polynom_order_intertrace_u, poly2d_params_l)      # replace the wavelength with the linear fit
                        if np.sum(good_values_r) > 10:
                            poly2d_params_r = polynomial_fit_2d_norm(x2[inorder2][good_values_r], y2[inorder2][good_values_r], arc_line_wave_io[good_values_r], 1, polynom_order_intertrace_u)    # linear fit in the outsides of the order
                            good_values_r = ( arc_lines_px_step[inorder2,1] > specs[1]-old_px_range+pxdiff )   # lines on the right, only covered by the new solution
                            #print(np.vstack([ arc_lines_px_step[good_values_r,1], arc_lines_px_step[good_values_r,0], arc_line_wave[good_values_r], 
                            #                  arc_line_wave_io[good_values_r] - polynomial_value_2d(x2[good_values_r], y2[good_values_r], 1, polynom_order_intertrace_u, poly2d_params_r) ]).T )
                            arc_line_wave_io[good_values_r] = polynomial_value_2d(x2[inorder2][good_values_r], y2[inorder2][good_values_r], 1, polynom_order_intertrace_u, poly2d_params_r)      # replace the wavelength with the linear fit
                    
                    ## Resolution in the central area
                    orders_in_y2 = np.unique(arc_lines_px_step[inorder2,0].astype(int))
                ...
                notnan  = ~np.isnan(arc_line_wave[:,:-1])
                notnanr = ~np.isnan(res_cen[:,:-1])
                nr_good = np.sum(notnan, axis=1)
                nr_good1 = np.where(nr_good == 1)[0]
                nr_good2 = np.where(nr_good > 1)[0]
                nr_goodr = np.sum(notnanr, axis=1)
                nr_good1r = np.where(nr_goodr == 1)[0]
                nr_good2r = np.where(nr_goodr > 1)[0]
                # Asign the values which are one
                arc_line_wave[nr_good1,-1] = np.nanmedian(arc_line_wave[nr_good1,0:-1], axis=1)     # as only one entry -> nanmedian just takes the value with one entry
                arc_line_wave[nr_good2,-1] = np.nanmedian(arc_line_wave[nr_good2,0:-1]*this_order[nr_good2,:], axis=1)       # Use only the values of this order
                arc_line_res[nr_good1,-1] = np.nanmedian(arc_line_res[nr_good1,0:-1], axis=1)     # as only one entry -> nanmedian just takes the value with one entry
                arc_line_res[nr_good2,-1] = np.nanmedian(arc_line_res[nr_good2,0:-1]*this_order[nr_good2,:], axis=1)       # Use only the values of this order
                res_cen[nr_good1r,-1] = np.nanmedian(res_cen[nr_good1r,0:-1], axis=1)     # as only one entry -> nanmedian just takes the value with one entry
                for order in nr_good2r:
                    for index_u, order_use in enumerate(orders_use):
                        if order in order_use:
                            res_cen[order,-1] = res_cen[order,index_u]       # use the value of the right order range
                            break
                ...
                orders_prev = list(orders_prev) + [np.min(orders_prev)-1, np.max(orders_prev)+1]      # Add only one order at the time
                arc_lines_wavelength = []
                oldorder = -1E5
                indexes = np.arange(arc_line_wave.shape[0])
                indexes = indexes[~np.isnan(arc_line_wave)]
                for ii in indexes:
                    order = int(arc_lines_px_step[ii,0])
                    if order not in orders_prev:                       # Ignore orders which are too far off
                        #if arc_lines_px_step[ii,0] != arc_lines_px_step[ii-1,0]:
                        #    print(11, order)
                        continue
                    if order != oldorder:                              # Each order the reference lines can be used again
                        assigned_reflines = []
                        oldorder = copy.copy(order)
                    # Ignore lines, for which the resolution is unphysical
                    if abs(arc_line_res[ii] - res_cen[order]) > res_cen[order] * 0.25:   # The variation in one order should be less than 15% (exohspec), 25% MRES
                        #if arc_lines_px_step[ii,0] != arc_lines_px_step[ii-1,0]:
                        #    print(22, order)
                        continue                # Ignore lines which differ a lot, hopefully other lines will fix that
                    # Wavelength difference to the catalog lines
                    diff_catalog = reference_catalog[:,0] - arc_line_wave[ii]    # Diff between catalog wavelength and fitted wavelength
                    diff_catalog[np.isnan(diff_catalog)] = 1E9
                    good_pos = np.where( np.abs(diff_catalog) <= max_diff_px*arc_line_res[ii] )[0]                   # Lines which are close to the fitted wavelength
                    # Remove already identified lines, can't be done earlier, as the index in good_pos is important
                    if only_one_line == True:
                        good_pos = np.array( [jj for jj in good_pos if jj not in assigned_reflines] )                  # Only keep the good_pos, which are not in assigned_reflines
                    ...
                    #if order <= 27:
                    #    print(ii, good_pos, arc_lines_px_step[ii,:], arc_line_res[ii], np.min(np.abs(diff_catalog)), max_diff_px )
                    for jj in good_pos:
                        #logdata[pos_logdata, 2+(step*iter_break+iteration)*2+1] = reference_catalog[j,0]
                        weight2 = np.log10(reference_catalog[jj,1])     # log10(intensity of the reference line)
                        arc_lines_wavelength.append([ arc_lines_px_step[ii,0], arc_lines_px_step[ii,1], reference_catalog[jj,0], diff_catalog[jj], 
                                                      weights1[ii], weight2, weights3[ii], arc_line_res[ii], jj, arc_lines_px_step[ii,2] ])
                        """arc_lines_wavelength[:,??] with ??:
                        0: order
                        1: pixel
                        ...
                        9: Gauss width of the line (in ThAr spectrum)
                        """
                        assigned_reflines.append(jj)                                                         # Don't use the line again
    ...
    for ii in range(20):                                                    # make this graphical!!
        ...
        for index_u, order_use in enumerate(orders_use):
            ...
            poly2d_params_u, new_waves = fit_wavelengths_solution_2d(arc_lines_wavelength[inorder & good_values,:], cen_pxs[inorder & good_values], order_offset, polynom_order_trace, polynom_order_intertrace)
            # Calculate the wavelengths again, using all values
            new_waves = polynomial_value_2d(arc_lines_wavelength[inorder,1]-cen_pxs[inorder], 1.0/(arc_lines_wavelength[inorder,0]+order_offset), polynom_order_trace, polynom_order_intertrace, poly2d_params_u)
            arc_lines_wavelength[inorder,3] = arc_lines_wavelength[inorder,2] - new_waves               # new differences
            poly2d_params.append(poly2d_params_u)
        if np.sum(good_values) < len_orig*2/3:
            break
    ...
    text = 'Info: A 2D polynomial fit with {0} orders in dispersion direction (along the traces) and {1} orders in cross-dispersion direction was used. '+\
                  ...
                  'With this values the standard deviation of the residuals between the central wavelengths and the grating equation is {3} Angstroms. Using the original solution gives an offset of {4}.'
    ...
    wavelength_solution2d = []
    for entry in poly2d_params:
    ...
    for order in orders:
        ...
        l.append('{0}'.format(order))
    text = ('Residuals of the identificated arc lines: identified {0} lines when using a 2D polynomial fit with {1} (dispersion) '+\
    ...
    for order in uniq_orders:
        ...
        for refname in uniq_refnames:
            ...
            if index_r != []:
                ...
                min_refline = np.min(data_o[index_r,positions[3]])
                max_refline = np.max(data_o[index_r,positions[3]])
                range_refline = np.max(data_o[:,positions[3]])-min(data_o[:,positions[3]])
                std = np.nan
...
def fit_basic_wavelength_solution(params, arc_lines_wavelength, spec, filename):
    ...
    for dummy in range(20):
        ...
        for i in range(arc_lines_wavelength.shape[0] - polynom_order_trace*polynom_order_intertrace):
            ...
            if np.max(arc_line_res) < 1:       # only good data left, probably should be less than 1 Angstrom
                break
    ...
    arc_line_res = np.abs(arc_line_wave_fit - arc_lines_wavelength[:,3])
    p_cen_px = np.round(p_cen_px,3)
    text = ('Info: The standard deviation of the residual of the fit to the manual wavelength solution is {1} Angstroms (average is {0} Angstroms). '+\
            'Only input data, for which the residuals were less than 1 Angstroms have been used. '+\
            '{2} of {5} lines have been used to calculate the solution for a 2D polynomial fit with {3} orders in dispersion direction (along the traces) '+\
...
def plot_gauss_data_center(datapoints_x, datapoints, label_datapoins, gauss, label_gauss, centroids, label_centroids, filename='', title='', poly=None, label_poly=None):
    ...
    for step in range(int((size[0]+9)/10)):          # Only plot 10 graphs in one image
        ...
        for i in datarange:
            x = np.linspace(min(datapoints_x[i,:]), np.max(datapoints_x[i,:]), len(datapoints_x[i,:])*20 )
...
def normalise_continuum(spec, wavelengths, nc=8, ll=2., lu=4., frac=0.3, semi_window=10, nc_noise=15):      # without modal noise nc=4,ll=1.,lu=5. might work
    ...
    """
    logger('Step: Normalise the continuum', show=False)
    specs = spec.shape
    ...
    for order in tqdm(range(specs[0]), desc='Step: Normalise the continuum'):
        #print "order", order
        ...
        sub[4,:] = copy.copy(sub[3,:])                                              # used to exclude to high residuals
        ori_len = np.sum(sub[3,:])
        if ori_len < 5:                                                     # Not enough useful data, but fill the arrays
        ...
        if np.nanstd(data[2,sub[0,:]]) < 1E-10:                                          # All values are the same 
            ccoefs.append( np.array( [0]*nc + [np.median(data[2,sub[0,:]])] ) )
        ...
        old_good_values = np.sum(sub[4,:])
        # Use only values that are +-0.9 Angstrom away from 
        data1_tile = np.tile(data[1,:], (specs[1],1))
        diff_array = np.abs( data1_tile - data1_tile.T )
        diff_array[np.isnan(diff_array)] = 100          # otherwise the next line create a runtime warning
        indexes_search_range = ( diff_array <= 0.9 )    #+- in Angstrom
        """get_timing('x2 ')
        ...
        for ii in data[0,sub[0,:]].astype(int):                                      # Only use values, for which the wavelength is available
            indexes_search_range[ii,sub[0,:]] = ( np.abs(data[1,sub[0,:]] - data[1,ii]) <= 0.9)    #+- in Angstrom
            #print i, sum(indexes_search_range[i,:]), data[1,sub[0,:]] - data[1,i]
        print(np.all(indexes_search_range == indexes_search_range1))
        get_timing('x3 ')"""
        frac_ori_len = frac*ori_len
        for step in range(1,int(old_good_values/10)):
            ...
            # [row, column];
            # np.sum(array, axis=1) is the same as {for i: np.sum(array[i,:])}, meaning rows stay the same;
            # tile(list, (3,1)) creates [list,list,list], meaning each row is the same
            """ not useful, as for loop changes values which are read again; also loop is not much slower
            tile_sub4 = np.tile(sub[4,:], (specs[1],1))
            revert_exclude = np.any( tile_sub4 & indexes_search_range, axis=1 )
            sub[4, (revert_exclude) & (~sub[4,:]*sub[1,:]) ] = True
            #sum_sub4 = np.sum( tile_sub4 & indexes_search_range, axis=1 )
            #sub[4,(sum_sub4==0) & (~sub[4,:]*sub[1,:])] = True 
            get_timing('x5 ')"""
            for i in data[0, ~sub[4,:]*sub[1,:]].astype(int):                       # Points that will be removed
                if not np.any( sub[4,indexes_search_range[i,:]] ):                 # Not enough data points remain in a given area
                    sub[4,i] = True                                                 # Don't create too big gaps as the high order polynomial nc>4 will not behave well there
            #print(np.all(temp==sub[4,:]))
            new_good_values = np.sum(sub[4,:])
            if ( np.sum(sub[4,:]) < frac_ori_len or new_good_values == old_good_values ) and step >= nc:         # stop if too much data is removed or no changes, but not if only linear equation is fitted
                ...
                break
            old_good_values = copy.copy(new_good_values)
        ccoefs.append(p)
        cen_waves.append(x_cen)
        ...
        for step in range(5):
            ...
            for i in data[0, ~sub[5,:]*sub[1,:]].astype(int):                   # Points that will be removed
                if np.sum( sub[5,indexes_search_range[i,:]] ) <= 0:             # Not enough data points remain in a given area
        ...
        for nc_step in range(1,nc_noise)[::-1]:                                 # Use a lower order, if fit remains stupid
            ...
            for dummy in range(100):
                with np.errstate(invalid='ignore'):
                    if np.any(~sub[5,:] & sub[1,:]):                                     # There are values that can be replaced
...
def linearise_wavelength_spec(params, wavelengths, spectra, method='sum', weight=[]):    
    ...
    :return : 1D array of wavelengths
    :return : 1D array of spectra
    """
    logger('Step: Linearise the spectrum.', show=False)
    specs = spectra.shape
    ...
    px_linspace1[0] = 0
    px_linspace2[-1] = specs[1]
    px_ranges = []
    for ii in range(px_linspace.shape[0]-1):
        px_range = np.arange(px_linspace[ii], px_linspace[ii+1])
        px_range_wide = np.arange(px_linspace1[ii], px_linspace2[ii+1])
        px_ranges.append([px_range, px_range_wide])
    wavelengths = copy.copy(wavelengths)
    spectra = copy.copy(spectra)
    nans = ( np.isnan(spectra) | np.isnan(wavelengths) )
    wavelengths[nans] = np.nan
    spectra[nans] = 0               # A nan will cause np.average to result in nan, even if the weight for the nan is 0
    wave_diff = np.nanmax(np.abs(wavelengths[:,1:] - wavelengths[:,:-1]))        # Difference in wavelength between 2 data points, abs needed for dodgy wavelength solutions
    if wave_diff > 1:
        diff = np.abs(wavelengths[:,1:] - wavelengths[:,:-1])
        pos = np.unravel_index(np.nanargmax(diff, axis=None), diff.shape)
        logger('Warn: unexpected wavelength step between two neighbouring pixel: {0} Angstroms in order {1}, pixel {2}. '.format(wave_diff, pos[0], pos[1])+\
               'Using a maximum wavelength step of 1.')
        wave_diff = 1
    minwave = np.nanmin(wavelengths, axis=1)                        # Minimum wavelength per order
    maxwave = np.nanmax(wavelengths, axis=1)                        # Maximum wavelength per order
    start = int(np.nanmin(minwave)/dwave) * dwave               # Minumum wavelength should a multiple of the wavelength_scale_resolution
    lwave_range = np.arange(start, np.nanmax(maxwave)+dwave, dwave )        # All wavelengths
    data_lin = np.zeros((specs[0], lwave_range.shape[0]))              # orders and wavelengths
    ...
    for order in tqdm(range(specs[0]), desc='Step: Linearise the spectrum'):
        for px_range, px_range_wide in px_ranges:           # splitting things into smaller junks (about 1k pixel instead of 6k pixel, not tested if there is an optimimum) speeds up the process by a factor of 5
            if np.all(nans[order,px_range]):
                continue
            px_range_sub = px_range[~nans[order,px_range]]
            px_range_wide_sub = px_range_wide[~nans[order,px_range_wide]]
            #print(px_range_sub.shape, wavelengths.shape)
            inorder = ( (lwave_range>=np.nanmin(wavelengths[order,px_range_sub])) & (lwave_range<=np.nanmax(wavelengths[order,px_range_sub])) )       # Save memory and CPU
            lwave_o = lwave_range[inorder]
            #get_timing('x1 ')
            diff_array = np.abs( np.tile(wavelengths[order,px_range_wide_sub], (lwave_o.shape[0],1)) - np.tile(lwave_o, (px_range_wide_sub.shape[0],1)).T )     # lwave in rows
            #get_timing('x2 ')
            toobig = (diff_array > wave_diff)
            weight_array = 1./(diff_array+0.01*dwave)
            weight_array[toobig] = 0       # A nan in either spectrum or weight will result in a nan
            no_data = np.all(weight_array==0, axis=1)
            weight_array[no_data,0] = 1             # At least one weight need to be different than 0
            spec_array = np.tile(spectra[order,px_range_wide_sub], (lwave_o.shape[0],1))
            data_lin[order, inorder] = np.average( spec_array, axis=1, weights=weight_array )
            if len(weight)>0:
                weigths_array = np.tile(weight[order,px_range_wide_sub], (lwave_o.shape[0],1))
                weight_comb = np.average( weigths_array, axis=1, weights=weight_array )
                weight_comb[no_data] = 0
    ...
    if method == 'sum':
        data_lin = np.nansum(data_lin, axis=0)
    elif method == 'mean':
        data_lin = np.nanmean(data_lin, axis=0)
    elif method == 'weight':
        weight_lin[0, nodata] = 1                               # add at a weight > 0
        data_lin = np.average(data_lin, axis=0, weights=weight_lin)
    else:
        logger('Warn: method {0} is not known, using sum'.format(method))
        data_lin = np.nansum(data_lin, axis=0)
    data_lin[nodata] = np.nan                               # Mark areas without data
    
    return lwave_range, data_lin
    
    """spectra[nans] = np.nan
    weight[nans] = np.nan
    a=copy.copy(lwave_range)
    ...
    for order in tqdm(range(specs[0]), desc='Step: Linearise the spectrum'):
        ...
        start = int(np.nanmin(wave_range)/dwave) * dwave          # Minumum wavelength should a multiple of the wavelength_scale_resolution
        lwave_range_o = np.arange(start, np.nanmax(wave_range)+dwave, dwave )
        ...
        lspec = f_spec(lwave_range_o)
        for wave in wave_range[~nonnan]:        # Ignore values which were not defined
            diff = np.abs(lwave_range_o - wave)   # Wavelength difference to the non defined values
            lspec[diff < wave_diff] = np.nan
        #plot_img_spec.plot_points([wave_range, wave_range, lwave_range_o], [spec, f_spec(wave_range), lspec], ['orig', 'spline', 'lin'], '', show=True, return_frame=False, x_title='wavelength [Angstrom]', y_title='flux [ADU]')
        lweight = np.ones(lwave_range_o.shape)
        if len(weight) > 0 :
            ...
            for i,wave in enumerate(lwave_range_o):
        ...
        data_o = np.ones(( lwave_range_o.shape[0], 3 ))
        data_o[:,0] = lwave_range_o
        data_o[:,1] = np.nan
        for i,wave in enumerate(lwave_range_o):
    ...
    print(a.shape, data_lin.shape, data.shape, np.all(a==data[:,0]), np.all(data_lin==data[:,1]), a, data_lin, data)
        ...
        if parentr == 'longitude':                  # Enough information to calculate the ephemerides of sun and moon.
            ...
            for obname in obnames:
                if obname.lower().find('sun') == 0:     # check with file_assignment.py: calibration_parameters_coordinates_UI(), creating the widgets
                    ...
                    source_radec = 'The object coordinates are derived from the calculated solar ephemeris.'
                    obnames.insert(0,'Sun')
                    break
                elif obname.lower().find('moon') == 0:
                    params['ra']          = mephem.ra
                    params['dec']         = mephem.dec
                    source_radec = 'The object coordinates are derived from the calculated lunar ephemeris.'
                    obnames.insert(0,'Moon')
                    break
                elif obname.lower().find('jupiter') == 0:
                    params['ra']          = jephem.ra
                    params['dec']         = jephem.dec
                    source_radec = 'The object coordinates are derived from the calculated Jupiters ephemeris.'
...
def convert_terra_master_hiflex(params, obname):
    ...
    if ii > 0:
        ...
        # Linear files
        if params['wavelength_scale_resolution'] > 0.0 and np.max(calimages.get('wave_sol_dict_cal',calimages['wave_sol_dict_sci'])['wavesol'][:,-1]) > 100:
        # Create a linearised solution for the input spectrum and the continuum corrected spectrum
            wavelengths_lin, spectrum_lin = linearise_wavelength_spec( params, spec[0], spec[5], method='weight', weight=((spec[1]+0.0)/spec[4])**2 ) # highest throughput and lowest error should have the highest weight
            save_multispec([wavelengths_lin,spectrum_lin], params['path_rv_terra']+im_name+'_lin_cont', im_head)
            save_linspec_csv(params['path_rv_terra']+im_name+'_lin_cont.csv', wavelengths_lin, spectrum_lin)
...
def convert_serval_master_hiflex(params, obname):
    ...
    if params['wavelength_scale_resolution'] > 0.0 and np.max(calimages.get('wave_sol_dict_cal',calimages['wave_sol_dict_sci'])['wavesol'][:,-1]) > 100:
        # Create a linearised solution for the input spectrum and the continuum corrected spectrum
        wavelengths_lin, spectrum_lin = linearise_wavelength_spec( params, spec[0], spec[5], method='weight', weight=((spec[1]+0.0)/spec[4])**2 ) # highest throughput and lowest error should have the highest weight
        save_multispec([wavelengths_lin,spectrum_lin], params['path_rv_serval']+im_name+'_lin_cont', im_head)
        save_linspec_csv(params['path_rv_serval']+im_name+'_lin_cont.csv', wavelengths_lin, spectrum_lin)
...
def stellar_params_ceres_multicore(kwargs):
    ...
    if np.sum(good_values) < 10:                               # If not enough values, then ignore the order
        good_orders = False
    else:
        good_orders = True
...
def rv_analysis_ceres(params, spec, fitsfile, obname, reffile, mephem, vsini):
    ...
    for cen_wave in [5130,6200,4000,7300,3000,8400,9500]:       # try different wavelengths in case one is not covered
        ...
        if len(subspec) > 0:
            ...
            break
        logger('Warn: The spectra around the wavelength of {0} Angstroms is not covered'.format(cen_wave))
